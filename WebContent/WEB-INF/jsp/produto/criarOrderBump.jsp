...
<template:admin>
	<form class="m-3 validateForm load" action="${sessao.urlPadrao}adm/orders-bump" method="post" role="form" enctype="multipart/form-data">
		...
		<div class="card-body card-padding">
				...
				<div class="form-group col-md-6">
			        ...
			        <select name="ordembump.produto.id" id="produtoBump" class="form-control required">
				    	...
				    	<c:forEach items="${produtosList}" var="produto">
				    		<option data-img="${produto.pegarPrimeiraImagem()}" value="${produto.id}"> ${produto.nome} </option>
				    	</c:forEach>
				    </select>
			    </div>			    
		    ...
		    <div class="form-group">
		           <button style="width:100%" type="submit" class="btn btn-primary sendForm">Cadastrar</button>	       
		    </div>
		</div>
	</form>
</template:admin>